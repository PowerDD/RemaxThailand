				...
				<div class="row">
					{section name=idx loop=$product.image}
						<div class="col-xs-3 col-sm-3 col-md-3">
							<img src="https://img.remaxthailand.co.th/100x100/product/{$product.sku}/{$product.image[idx]}" alt="{$product.name}" class="pointer img-thumbnail img_small_list{if $product.image[idx] == $product.cover} active{/if}">
						</div>
					{/section}
				</div>
			{/if}
			<div class="row margin-top-15">
				...
				{if $product.standard != ''}
					<div class="col-xs-12 text-center">
						{if $product.standard|strpos:'CE' !== false}<img src="https://src.remaxthailand.co.th/img/web/icon/logo-ce.png">{/if}
						{if $product.standard|strpos:'FCC' !== false}<img src="https://src.remaxthailand.co.th/img/web/icon/logo-fcc.png"?\>{/if}
						{if $product.standard|strpos:'RoHS' !== false}<img src="https://src.remaxthailand.co.th/img/web/icon/logo-rohs.png">{/if}
					</div>
				{/if}
			</div>
			...
			<div class="pull-left">
				{if isset($product.warranty) && $product.warranty > 0}
					<div><span class="label label-success">รับประกันสินค้า 
						{if $product.warranty >= 365}{$product.warranty/365} ปี{else if $product.warranty >= 30}{$product.warranty/30} เดือน{else}{$product.warranty} วัน{/if}</span>
						<span class="small"> โดย Remax (Thailand)</span></div>
				{/if}
			...
			<div class="row margin-top-10">
				<div class="col-sm-12"><div class="line"></div></div>
					...
					<div class="col-sm-6 margin-top-10">
						...
						<div class="small">จัดจำหน่ายและดำเนินการโดย Remax (Thailand)</div>
					</div>
						...
						{if $product.imageDetail|count > 0}
							<div class="row margin-top-15">
								{section name=idx loop=$product.imageDetail}
									<div class="col-sm-12">
										<img src="http://img.remaxthailand.co.th/product/{$product.sku}/{$product.imageDetail[idx]}" alt="{$product.name}" class="img-responsive no-radius">
									</div>
								{/section}
							</div>
						{/if}
